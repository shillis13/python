...
"""
def message_contains_keywords(message: Dict[str, Any], keywords: List[str], case_sensitive: bool) -> bool:
    if not keywords:
        return True
...
"""
def filter_messages(messages: List[Dict[str, Any]], config: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    filtered = []
    ...
    for i, message in enumerate(messages):
        ...
        # Index filtering
        if 'keep_indices' in config and message_index not in expand_open_ranges(config['keep_indices'], len(messages)):
            include = False
        if 'remove_indices' in config and message_index in expand_open_ranges(config['remove_indices'], len(messages)):
            include = False

        # Keyword filtering
        if 'include_keywords' in config and not message_contains_keywords(message, config['include_keywords'], config.get('case_sensitive', False)):
            include = False
        if 'exclude_keywords' in config and message_contains_keywords(message, config['exclude_keywords'], config.get('case_sensitive', False)):
            include = False
...
def show_file_analysis(file_paths: List[str], list_only: bool = False) -> None:
    if len(file_paths) == 1:
        ...
        if list_only:
            print(f"All available roles across {total_files} files: {', '.join(sorted(all_roles))}")
        else:
...
"""
def validate_roles(requested_roles: List[str], available_roles: List[str]) -> Tuple[List[str], List[str]]:
    valid_roles = []
    invalid_roles = []
...
"""
def process_single_file(input_file: Path, output_file: Path, args: argparse.Namespace) -> bool:
    try:
        with open(input_file, 'r', encoding='utf-8') as f:
            chat_data = json.load(f)
        ...
        if args.roles:
            ...
            if analysis:
                ...
                valid_roles, invalid_roles = validate_roles(requested_roles, analysis['available_roles'])
                
                if invalid_roles:
                    print(f"❌ Error in {input_file.name}: Role(s) not found: {', '.join(invalid_roles)}", file=sys.stderr)
                    print(f"   Available roles: {', '.join(analysis['available_roles'])}", file=sys.stderr)
                    
                    # Suggest similar roles
                    for invalid_role in invalid_roles:
                        suggestions = [role for role in analysis['available_roles'] 
                                     if invalid_role.lower() in role.lower() or role.lower() in invalid_role.lower()]
                        if suggestions:
        ...
        if args.include_kw: 
            filter_config['include_keywords'] = [k.strip() for k in args.include_kw.split(',')]
        if args.exclude_kw: 
            filter_config['exclude_keywords'] = [k.strip() for k in args.exclude_kw.split(',')]
        ...
        print(f"ℹ️ Filtered {input_file.name}: {stats['original_count']} -> {stats['filtered_count']} messages.", file=sys.stderr)
        return True
...
Examples:
    ...
    parser.add_argument('-o', '--output-dir', help='Output directory for generated files (default: same as input).')
    parser.add_argument('-ff', '--from-file', help='Read file paths from a text file.')
    
    # Analysis options
    analysis_group = parser.add_mutually_exclusive_group()
    analysis_group.add_argument('--list-roles', action='store_true', help='List available roles in the file(s) and exit.')
    analysis_group.add_argument('--stats', action='store_true', help='Show detailed file analysis and exit.')
    
    # Filtering arguments
    index_group = parser.add_mutually_exclusive_group()
    index_group.add_argument('--keep-msgs', help='Keep specific message indices (e.g., "1-10,20,50-").')
    index_group.add_argument('--remove-msgs', help='Remove specific message indices (e.g., "5,10-15").')
    parser.add_argument('--roles', help='Comma-separated roles to keep (e.g., "user,assistant").')
    parser.add_argument('--include-kw', help='Comma-separated keywords to require in messages.')
    parser.add_argument('--exclude-kw', help='Comma-separated keywords to exclude messages.')
    parser.add_argument('--case-sensitive', action='store_true', help='Make keyword matching case-sensitive.')
    parser.add_argument('--min-length', type=int, help='Minimum character length for messages.')
    parser.add_argument('--max-length', type=int, help='Maximum character length for messages.')
    parser.add_argument('--min-words', type=int, help='Minimum word count for messages.')
    parser.add_argument('--max-words', type=int, help='Maximum word count for messages.')
    ...
    filter_args = ['keep_msgs', 'remove_msgs', 'roles', 'include_kw', 'exclude_kw', 'min_length', 'max_length', 'min_words', 'max_words']
    if not any(getattr(args, fa) for fa in filter_args):
        parser.error("At least one filtering argument is required (e.g., --roles, --keep-msgs). Use --list-roles or --stats for analysis.")
