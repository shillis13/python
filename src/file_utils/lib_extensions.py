...
def compare_extensions(
    csv_extensions: Iterable[str], magic_extensions: Iterable[str]
...
def _parse_hierarchy_recursive(
    data: dict, parent: str | None, type_map: dict, ext_map: dict
):
    for type_name, type_data in data.items():
        ...
        if "sub_types" in type_data:
            _parse_hierarchy_recursive(
                type_data["sub_types"], type_name, type_map, ext_map
            )
...
def print_type_query(
    type_name: str,
    data: dict,
    ancestors: bool,
    descendants_tree: bool,
    descendants_flat: bool,
):
