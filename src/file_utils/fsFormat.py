...
def _row_from_file_info(
    ...
    *,
    ...
    path_value = (
        path_formatter(file_info.path) if path_formatter else str(file_info.path)
    )
    parent_value = (
        parent_formatter(file_info.path.parent)
        if parent_formatter
        else str(file_info.path.parent)
    )
    ...
    return {
        ...
        "type": (
            ...
            else ("dir" if file_info.is_dir else "file")
        ),
        "kind": file_info.kind,
...
AVAILABLE_COLUMNS: Dict[str, ColumnSpec] = {
    ...
    "size_bytes": ColumnSpec(
        "size_bytes", "Bytes", lambda row: row["size_bytes"], "right", 6
    ),
    "modified": ColumnSpec(
        "modified", "Modified", lambda row: row["modified"], "left", 16
    ),
    "modified_iso": ColumnSpec(
        "modified_iso", "Modified ISO", lambda row: row["modified_iso"], "left", 19
    ),
    "created": ColumnSpec("created", "Created", lambda row: row["created"], "left", 16),
    "created_iso": ColumnSpec(
        "created_iso", "Created ISO", lambda row: row["created_iso"], "left", 19
    ),
    "accessed": ColumnSpec(
        "accessed", "Accessed", lambda row: row["accessed"], "left", 16
    ),
...
def _column_help_text() -> str:
    """Build the help text for --columns with the available column keys."""
    available = ", ".join(sorted(AVAILABLE_COLUMNS.keys()))
    return (
        "Comma-separated list of columns for list/table/CSV formats. "
        f"Available columns: {available}. Use 'all' to include every column."
    )


def resolve_columns(
    requested: Optional[Sequence[str]], default: Sequence[str] = DEFAULT_LIST_COLUMNS
) -> List[ColumnSpec]:
    ...
    *,
    ...
    if wrap_mode == "word":
        wrapped = textwrap.wrap(
            value, width=width, break_long_words=False, drop_whitespace=False
        )
        return wrapped or [""]
...
class FileSystemFormatter:
    ...
    def __init__(
        ...
        if path_style not in {
            ...
            raise ValueError(
                "path_style must be one of 'relative', 'absolute', 'relative-start', or 'relative-home'"
            )
        if trim_mode not in {"left", "right", "center"}:
            raise ValueError("trim_mode must be 'left', 'right', or 'center'")
        ...
        # Statistics
        self.stats = {"dirs": 0, "files": 0, "symlinks": 0, "total_size": 0}
    ...
    def _format_relative(
        self, path: Path, base: Optional[Path], *, allow_relpath: bool = False
    ) -> Optional[str]:
        if base is None:
            return None
        try:
    ...
    def format_path(self, path: Path) -> str:
        ...
        if style == "relative-home":
            home_relative = self._format_relative(
                absolute, self.home_path, allow_relpath=False
            )
        ...
        if style == "relative-start":
            ...
            if relative is not None:
                return relative
            style = "relative"
        relative_default = self._format_relative(
            absolute, self.default_relative_base, allow_relpath=True
        )
    ...
    def _sort_file_infos(self, items: List[FileInfo]) -> List[FileInfo]:
        ...
        sorted_items = sorted(
            items, key=self._get_sort_value, reverse=self.reverse_sort
        )

        if self.sort_dirs_first:
            sorted_items = sorted(
                sorted_items, key=lambda item: 0 if item.is_dir else 1
            )

        return sorted_items

    def get_sorted_children(
        self, directory: Path, fs_filter: FileSystemFilter = None
    ) -> List[FileInfo]:
    ...
    def _collect_from_directory(
        ...
        directory: Path,
        items: List[FileInfo],
        fs_filter: FileSystemFilter = None,
        depth: int = 0,
    ):
        ...
        for child_info in children:
            if child_info.is_dir:
                self._collect_from_directory(
                    child_info.path, items, fs_filter, depth + 1
                )
            else:
        ...
        for i, child_info in enumerate(children):
            ...
            if child_info.is_dir and not child_info.is_symlink:
                self._format_tree_recursive(
                    child_info.path, next_prefix, depth + 1, fs_filter, output
                )
    ...
    def print_summary(self):
        ...
        if self.format_type == "tree":
            ...
            if self.stats["dirs"] > 0:
                parts.append(
                    f"{self.stats['dirs']} director{'ies' if self.stats['dirs'] != 1 else 'y'}"
                )
            if self.stats["files"] > 0:
                parts.append(
                    f"{self.stats['files']} file{'s' if self.stats['files'] != 1 else ''}"
                )
            if self.stats["symlinks"] > 0:
                parts.append(
                    f"{self.stats['symlinks']} symlink{'s' if self.stats['symlinks'] != 1 else ''}"
                )
...
def add_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--path-base", help="Base directory to use when --path-style=relative-start"
    )
    ...
    parser.add_argument(
        "--tree", action="store_const", const="tree", dest="format", help="Tree format"
    )
    parser.add_argument(
        "--table",
        action="store_const",
    ...
    parser.add_argument(
        "--json", action="store_const", const="json", dest="format", help="JSON format"
    )
    parser.add_argument(
        "--yaml", action="store_const", const="yaml", dest="format", help="YAML format"
    )
    parser.add_argument(
        "--csv", action="store_const", const="csv", dest="format", help="CSV format"
    )
    parser.add_argument(
        "--legacy-output",
        action="store_true",
    ...
    parser.add_argument(
        "--max-depth", "-d", type=int, metavar="N", help="Maximum depth to descend"
    )
    parser.add_argument(
        "--hidden", action="store_true", help="Show hidden files and directories"
    )

    # Display options
    parser.add_argument("--size", "-s", action="store_true", help="Show file sizes")
    parser.add_argument(
        "--modified", "-m", action="store_true", help="Show modification dates"
    )
    parser.add_argument(
        "--permissions", "-p", action="store_true", help="Show file permissions"
    )
    parser.add_argument("--columns", "-col", help=_column_help_text())
    parser.add_argument(
        "--wrap",
        choices=["none", "word", "truncate"],
        default="truncate",
    ...
    parser.add_argument(
        "--ascii", "-a", action="store_true", help="Use ASCII characters for tree"
    )
    parser.add_argument(
        "--no-colors", action="store_true", help="Disable colored output"
    )
    parser.add_argument(
        "--unsorted", action="store_true", help="Don't sort directories before files"
    )
    parser.add_argument(
        "--no-summary", action="store_true", help="Don't show summary statistics"
    )

    # Grouping and sorting
    parser.add_argument(
        "--group-by",
        choices=["type", "extension", "size", "date"],
        help="Group items by specified attribute",
    )
    parser.add_argument(
        "--sort-by",
        choices=["name", "size", "modified", "type", "kind"],
        default="name",
        help="Sort items by specified attribute",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pattern", "-pat", help="Pattern for both files and directories"
    )
    parser.add_argument(
        "--file-pattern",
        "-fp",
    ...
    parser.add_argument(
        "--type", "-t", action="append", default=[], help="File types to include"
    )
    parser.add_argument(
        "--extension",
        "-e",
    ...
    parser.add_argument(
        ...
        "-q",
        action="store_true",
        help="Suppress commentary and only print formatted results",
    )
...
EXECUTION OPTIONS:
    ...
    --quiet, -q           Suppress commentary and only print formatted results
...
def process_format_pipeline(args):
    ...
    if not input_paths:
        if piped_input:
            return  # Nothing flowed through the pipe, so stay silent in pipelines.
    ...
    if args.col_widths:
        for entry in args.col_widths.split(","):
            ...
            if "=" not in entry:
                raise ValueError(
                    f"Invalid column width specification '{entry}'. Expected name=value"
                )
            ...
            try:
                ...
                raise ValueError(
                    f"Column width for '{name}' must be an integer"
                ) from exc
    ...
    try:
        ...
        if args.format == "tree" and not args.no_summary and not args.quiet:
    ...
    if not args.quiet:
        ...
        print(
            f"✅ Formatted {total_paths} path{'s' if total_paths != 1 else ''} as {format_name}.",
            file=sys.stderr,
        )
