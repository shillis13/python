...
DEFAULT_DATA_DIR = (
    Path(os.environ.get("CHAT_HISTORY_DATA_DIR", Path(__file__).parent)).parent / "data"
)
...
def convert_export_to_history_v13(export: dict) -> Dict[str, Any]:
    ...
    total_exchanges = sum(
        1 for m in history_messages if m["role"] in ("user", "assistant")
    )

    participants = sorted({m["role"] for m in history_messages})
    participants_rec = [
        {"participant_id": f"p_{i+1}", "role": r, "name": None}
...
def make_index_entry_strict13(
    history: Dict[str, Any], file_path: str
) -> Dict[str, Any]:
...
def main():
    ...
    ap.add_argument(
        "--history-schema",
        default=str(DEFAULT_DATA_DIR / "chat_history_schema_v1.3.yml"),
    )
    ap.add_argument(
        "--index-schema", default=str(DEFAULT_DATA_DIR / "chat_index_schema_v1.3.yml")
    )
    args = ap.parse_args()
    ...
    history_path.write_text(
        yaml.safe_dump(history, sort_keys=False, allow_unicode=True), encoding="utf-8"
    )
    ...
    print(
        "History validation:", "OK" if not hist_problems else f"Issues: {hist_problems}"
    )
