...
def parse_markdown_chat(file_path):
    ...
    try:
        ...
        is_chat_log = re.search(r'^\s*(user|assistant|system)\s*:', content, re.IGNORECASE | re.MULTILINE)

        if is_chat_log:
            # Split content into messages based on roles
            # This regex splits by "role:" case-insensitively
            parts = re.split(r'^\s*(user|assistant|system)\s*:\s*', content, flags=re.IGNORECASE | re.MULTILINE)
            if len(parts) > 1:
...
def to_html(metadata, messages, css_content):
    ...
    body_markdowner = markdown2.Markdown(extras=["tables", "fenced-code-blocks", "strike"])
