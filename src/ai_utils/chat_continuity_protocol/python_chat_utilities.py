...
class ChatHistoryManager:
    ...
    def record_claude_response(
        self, content: str, artifact_ids: List[str] = None, tags: List[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def start_new_session(
        self, continued_from: Optional[str] = None, tags: List[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        self.history["metadata"].update(
            {
                ...
                "previous_conversation_id": imported_data["metadata"][
                    "conversation_id"
                ],
    ...
    def export_to_yaml(self) -> str:
        ...
        return yaml.dump(
            self.history, default_flow_style=False, indent=2, sort_keys=False
        )
    ...
    def export_for_handoff(self) -> str:
        ...
        return yaml.dump(
            handoff_data, default_flow_style=False, indent=2, sort_keys=False
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        estimated_tokens = sum(
            msg["metadata"]["estimated_tokens"] for msg in all_messages
        )
...
Examples:
    ...
    parser.add_argument(
        "yaml_file", nargs="?", type=Path, help="YAML chat history file to work with"
    )
    parser.add_argument(
        "--init", action="store_true", help="Initialize new chat history file"
    )
    parser.add_argument(
        "--add-message", action="store_true", help="Add a message to the chat history"
    )
    parser.add_argument(
        "--role",
    ...
    parser.add_argument(
        "--stats", action="store_true", help="Show conversation statistics"
    )
    ...
    try:
        ...
        if args.init:
            ...
            stats = chat.get_stats()
            print(
                f"📄 {args.yaml_file}: {stats['total_messages']} messages, {stats['estimated_tokens']} tokens"
            )
