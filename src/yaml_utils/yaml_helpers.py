...
def archive_and_update_metadata(data, parent_key_str):
    ...
    if parent_key not in data:
        print(f"Error: Parent key '{parent_key}' not found for archiving.", file=sys.stderr)
        sys.exit(1)
    ...
    if 'metadata' in data[parent_key]:
        data[parent_key]['metadata']['version'] = data[parent_key]['metadata'].get('version', 0) + 1
        data[parent_key]['metadata']['last_updated'] = datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')
