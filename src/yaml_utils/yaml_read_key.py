...
def delete_yaml_item(data, key_path, index):
    ...
    if abs(index) >= len(current_level):
        raise IndexError(
            f"Error: Index {index} is out of bounds for list of size {len(current_level)}."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Delete an item from a list by its index."
    )
    parser.add_argument("filepath", help="Path to the YAML file.")
    parser.add_argument(
        "--key", required=True, help="The key of the list (e.g., 'rules.data')."
    )
    parser.add_argument(
        "--index", required=True, type=int, help="The index of the item to delete."
    )
