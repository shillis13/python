...
def analyze_yaml_comprehensive(data: Any) -> Dict[str, Any]:
    ...
    def _analyze_recursive(obj: Any, current_path: str = "", depth: int = 0) -> None:
        analysis["structure"]["max_depth"] = max(
            analysis["structure"]["max_depth"], depth
        )
    ...
    def _analyze_value(value: Any) -> None:
        ...
        if isinstance(value, str):
            ...
            analysis["content"]["value_lengths"]["distribution"][
                value_len // 10 * 10
            ] += 1
    ...
    if analysis["structure"]["total_values"] > 0:
        ...
        total_depth = sum(
            d * c for d, c in analysis["structure"]["depth_distribution"].items()
        )
        total_nodes = sum(analysis["structure"]["depth_distribution"].values())
        analysis["structure"]["avg_depth"] = (
            total_depth / total_nodes if total_nodes > 0 else 0
        )
