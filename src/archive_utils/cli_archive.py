...
def setup_create_parser(subparsers):
    ...
    create_parser.add_argument(
        "archive_name", help="The name of the archive file to create."
    )
    create_parser.add_argument(
        "directories",
        nargs="*",
        help="Files or directories to add. If omitted, reads from stdin or --from-file.",
    )
    ...
    create_parser.add_argument(
        ...
        const=True,
        default=None,
        help="Encrypt the zip archive.",
    )
    create_parser.add_argument(
        "--compression",
        choices=["deflate", "bzip2", "lzma", "store"],
        default="deflate",
        help="Compression method for zip files.",
    )
...
def handle_create(args):
    ...
    lib_logging.log_info(
        f"Starting 'create' operation for archive: {args.archive_name}"
    )
    ...
    try:
        if args.type == "tar" and password:
            lib_logging.log_info(
                "WARNING: Password protection is not supported for .tar files."
            )
        ...
        lib_logging.log_info(
            f"SUCCESS: Successfully created archive: {args.archive_name}"
        )
...
def setup_extract_parser(subparsers):
    ...
    extract_parser.add_argument(
        "destination", nargs="?", default=".", help="Directory to extract to."
    )
...
def handle_extract(args):
    ...
    lib_logging.log_info(
        f"Starting 'extract' operation for archive: {args.archive_name}"
    )
    password = None
    if args.password:
        password = args.password
    ...
    try:
        ...
        lib_logging.log_info(
            f"SUCCESS: Successfully extracted archive to: {args.destination}"
        )
    except Exception as e:
