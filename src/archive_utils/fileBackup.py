...
def add_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "files",
        nargs="*",
        help="Paths of files to back up, supports wildcards like *.py.",
    )
    parser.add_argument(
        "--archive-dir",
        default="Archive",
...
def process_files_pipeline(args):
    ...
    print(
        f"✅ {operation_type} {successful_backups} files to '{args.archive_dir}'.",
        file=sys.stderr,
    )
...
def main():
    ...
    if args.files and sys.stdin.isatty() and not args.from_file:
        ...
        operation_type = "Would backup" if args.dry_run else "Successfully backed up"
        print(
            f"✅ {operation_type} {len(backup_info)} files to '{args.archive_dir}'.",
            file=sys.stderr,
        )
    else:
        # Pipeline mode
