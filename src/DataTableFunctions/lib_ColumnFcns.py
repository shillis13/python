...
def add_columns(df, columns):
    for col_def in columns:
        ...
        if col_name not in df.columns:
            ...
            if dtype == "number":
                df.loc[:, col_name] = df.loc[:, col_name].astype(
                    float
...
def remove_other_columns(df, columns_to_retain, exact_match=True):
    if exact_match:
        ...
        retained_columns = [
            col for col in df.columns if any(sub in col for sub in columns_to_retain)
        ]
...
def test_Fcn_CountUnique():
    ...
    actual_non_unique_parentkey = Fcn_CountUnique(
        df, ["ParentKey"], count_non_unique=True
    )
    if actual_non_unique_parentkey != expected_non_unique_parentkey:
        all_tests_passed = False
        print(
            f"Test Case 1 Failed: Expected {expected_non_unique_parentkey}, but got {actual_non_unique_parentkey}"
        )
    ...
    if actual_unique_parentkey != expected_unique_parentkey:
        all_tests_passed = False
        print(
            f"Test Case 2 Failed: Expected {expected_unique_parentkey}, but got {actual_unique_parentkey}"
        )

    actual_non_unique_parent_child = Fcn_CountUnique(
        df, ["ParentKey", "ChildKey"], count_non_unique=True
    )
    if actual_non_unique_parent_child != expected_non_unique_parent_child:
        all_tests_passed = False
        print(
            f"Test Case 3 Failed: Expected {expected_non_unique_parent_child}, but got {actual_non_unique_parent_child}"
        )
    ...
    if actual_unique_parent_child != expected_unique_parent_child:
        all_tests_passed = False
        print(
            f"Test Case 4 Failed: Expected {expected_unique_parent_child}, but got {actual_unique_parent_child}"
        )
