...
def chunk_single_file(
    input_file: Path, output_dir: Path, max_chars: int, overlap: int
) -> List[Path]:
    ...
    try:
        ...
        if not messages:
            print(
                f"⚠️ No 'messages' found in {input_file.name}, skipping.",
                file=sys.stderr,
            )
            return []
        ...
        while current_start < len(messages):
            ...
            print(
                f"ℹ️ Created chunk {output_file.name} with {len(chunk_messages)} messages.",
                file=sys.stderr,
            )
...
def process_files_pipeline(args: argparse.Namespace) -> None:
    ...
    for file_path_str in file_paths:
        ...
        chunk_output_dir = (
            output_dir if output_dir else input_file.parent / input_file.stem
        )

        chunk_files = chunk_single_file(
            input_file, chunk_output_dir, args.size, args.overlap
        )
...
def main():
    ...
    parser.add_argument(
        "-o",
        "--output-dir",
        help="Output directory for generated chunk folders (default: creates sub-folder next to input).",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of messages to overlap between chunks (default: 2).",
    )
